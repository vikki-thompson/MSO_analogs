# Calculates 2D streamfunction field from u- and v- 2D wind fields.
...
def generate_yrmonths():
	years = range(1940, 2024) # Full range: (1940,2024)
	months = ['01','02','03','04','05','06','07','08','09','10','11','12']
	yrmonths = [ int("%s%s" % (year,month)) for year in years for month in months]
	return yrmonths

yrmonths = generate_yrmonths()
#yrmonths = ['202304','202305','202306','202307','202308'] # use this line to select specific YYYYMM (can be list)
...
for yrmonth in yrmonths:
    ...
    cdo_cmd = ['cdo','-b','F32','sp2gp', '-dv2ps', '-uv2dv', '-remapbil,F360', '-merge', u_wind, v_wind, tmpfile] # this is the line that does the calculation
    print(' '.join(cdo_cmd))
    ret = subprocess.call(cdo_cmd)
    ret = subprocess.call(cdo_cmd)
    if not ret==0:
        raise Exception('Error with first cdo command')
    cdo_cmd = ['cdo','selvar,stream', tmpfile, outfile] # Select the required variable (n.b. if loaded as an iris.cube metadata reads wrong)
    print(' '.join(cdo_cmd))
    ret = subprocess.call(cdo_cmd)
    if not ret==0:
