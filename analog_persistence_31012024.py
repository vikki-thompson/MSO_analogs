...
#
# Assessing persistence, using spatial correlation
#
...
### PERSISTENCE OF EVENT Eucledian distance in year of event
psi_event = gdata.pull_out_day_era(gdata.extract_region(gdata.extract_JJA(gdata.era5_mydata('psi250', range(2021,2023))), region), date[0], date[1], date[2])
...
Pevent = persistence('2021714', reg, coeff)

P_past = []
for each_date in dates_past:
    P_past.append(persistence(each_date, reg, coeff))

P_present = []
for each_date in dates_present:
    P_present.append(persistence(each_date, reg, coeff))  
...
psi_lentis = gdata.extract_region(gdata.extract_JJA(gdata.lentis_mydata()), reg)
psi_lentis_future = gdata.extract_region((gdata.extract_JJA(gdata.lentis_mydata(run='F'))), reg)
...
def persistence_lentis(psi, date, region, coeff):
    ...
    ens = np.int(date[:3])
    yr = np.int(date[3:7])
    mon = calendar.month_abbr[int(date[7])]
...
# LENTIS present
PL_present = []
psi = gdata.extract_region(gdata.extract_JJA(gdata.lentis_mydata()), reg)   
for each_date in Z[:30]:
    PL_present.append(persistence_lentis(psi, each_date, reg, coeff))
    print(PL_present)

PL_future = []
psi = gdata.extract_region((gdata.extract_JJA(gdata.lentis_mydata(run='F'))), reg)
for each_date in Z_future[:30]:
    PL_future.append(persistence_lentis(psi, each_date, reg, coeff))  
    print(PL_future)
'''
...
## Significance of difference between time periods
...
all_ax = axs.ravel()
for i in range(30):
    field = gdata.pull_out_day_era(psi, year, mon, i+1)
    c = all_ax[i].contourf(lons, lats, field.data, levels=np.linspace(event_min, event_max, 20), cmap = plt.cm.get_cmap('RdBu_r'), transform=ccrs.PlateCarree(), extend='both')
    #pdata.plot_box(all_ax[i], R1) # plot box of analog region
    all_ax[i].add_feature(cf.BORDERS)
    all_ax[i].add_feature(cf.COASTLINE)
    all_ax[i].set_title('{0:.2f}'.format(pearsonr(psi_event.data.reshape(40*61), field.data.reshape(40*61))[0]))
    

dates_present[2] # P = 7
dates_present[9] # P = 13
## 14 day persistence - maps
for each in dates_present[:10]:
    date = each
    reg = R1
    year = np.int(date[:4])
    mon = calendar.month_abbr[int(date[4])]
