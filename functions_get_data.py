...
def era5_mon_data(var, years=np.arange(1959,2023)):
    ...
    for yr in years:
        print(yr)
        files = glob.glob('/net/pc170547/nobackup_2/users/sager/ERA5/'+str(yr)+'/mon/era5_'+var+'_*') # numbers indicate months extracted
...
def composite_max_dates(psi, date_list):
    '''
    Returns single composite of grid point max all dates
    Inputs required:
      psi = list of cubes, 1 per year - as used to calc D/date_list
      date_list = list of events to composite
    '''
